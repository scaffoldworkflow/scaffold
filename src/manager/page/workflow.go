...
func WorkflowPageEndpoint(ctx *gin.Context) {
	markdown := workflowBuildPage(ctx)
	ctx.Data(http.StatusOK, "text/html; charset=utf-8", markdown)
}

func workflowBuildPage(ctx *gin.Context) []byte {
		...
		Components: []ui.Component{
			...
			div.Div{
				...
				Components: []ui.Component{
						...
						Components: []ui.Component{
							breadcrumb.Breadcrumb{
								Components: []ui.Component{
									...
									link.Link{
										Title: "Workflows",
										HRef:  fmt.Sprintf("/ui/releases/%s/%s/%s", releaseID, runID, serviceName),
									},
									link.Link{
										Title: serviceName,
										HRef:  fmt.Sprintf("/ui/releases/%s/%s/%s", releaseID, runID, serviceName),
							...
							// },
							ui.Raw{
								HTMLString: `<input id="search" class="w3-input  search-bar dark theme-light" type="text" name="search" placeholder="Search Tasks" style="margin-left:5%;margin-top:8px;margin-bottom:8px;width:90%;" oninput="render()">`,
							},
						},
					},
					card.Card{
						...
						// Components: []ui.Component{
						// workflowBuildTabs(tName, pName, eName),
						// },
					},
					br.BR{},
					// modal.Modal{
					...
					// 			<span onclick="saveInputs()" class="w3-button dark theme-light " style="margin-top:5px;float:right;margin-right:8px;"><i class="fa-solid fa-floppy-disk" id="save-icon"></i></span>
					// 			`,
					// 			Classes: "ui-green",
					// 			Style:   "width:100%;",
					// 		},
					...
					modal.Modal{
						...
						Components: []ui.Component{
							h1.H1{
								...
								<!-- <span onclick="killRun()" class="w3-button dark theme-light " style="margin-top:5px;float:right;margin-right:8px;"><i class="fa-solid fa-stop"></i></span> -->
								<!-- <span onclick="triggerRun()" class="w3-button dark theme-light " style="margin-top:5px;float:right;margin-right:8px;"><i class="fa-solid fa-play"></i></span> -->
								<!-- <span onclick="toggleDisable()" class="w3-button dark theme-light " style="margin-top:5px;float:right;margin-right:8px;"><i class="fa-solid fa-toggle-on" id="toggle-icon"></i></span> -->
								`,
								Classes: "ui-green",
								Style:   "width:100%;",
							},
							...
							collapse.Collapse{
								...
								Components: []ui.Component{
									br.BR{},
									ui.Raw{
										HTMLString: `<div class="mockup-code" id="state-output"></div>`,
									},
									// pre.Pre{
									// 	ID:    "state-output",
...
func workflowBuildTabs(tName, pName, eName string) ui.Component {
	envs, err := project.GetEnvironments(bson.M{"team": tName, "project": pName, "name": eName})
	if err != nil {
		logger.Errorf("", "Could not get environment at %s/%s/%s: %s", tName, pName, eName, err)
		return div.Div{}
	}
	if len(envs) == 0 {
	...
	tabs := `<div role="tablist" class="tabs tabs-lifted">`
	first := true
	for name, _ := range envs[0].Services {
		if first {
			tabs += fmt.Sprintf(`<a role="tab" class="tab tab-active">%s</a>`, name)
			first = false
			continue
		}
		tabs += fmt.Sprintf(`<a role="tab" class="tab">%s</a>`, name)
	}
	tabs += `</div>`
	return ui.Raw{HTMLString: tabs}
}
