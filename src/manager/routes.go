...
func initializeRoutes() {
	router.Static("/static/css", "./page/static/css")
	router.Static("/static/img", "./page/static/img")
	router.Static("/static/js", "./page/static/js")
	...
	apiRoutes := router.Group("/api", middleware.CORSMiddleware())
	{
		v1Routes := apiRoutes.Group("/v1")
		{
			...
			environmentRoutes := v1Routes.Group("/environment")
			{
				environmentRoutes.GET("", middleware.EnsureLoggedIn(), api.GetEnvironments)
				environmentRoutes.DELETE("", middleware.EnsureLoggedIn(), api.DeleteEnvironments)
				environmentRoutes.POST("", middleware.EnsureLoggedIn(), api.CreateEnvironment)
				environmentRoutes.PUT("", middleware.EnsureLoggedIn(), api.UpdateEnvironment)
			}
			serviceRoutes := v1Routes.Group("/service")
			{
				serviceRoutes.GET("", middleware.EnsureLoggedIn(), api.GetServices)
				serviceRoutes.DELETE("", middleware.EnsureLoggedIn(), api.DeleteServices)
				serviceRoutes.POST("", middleware.EnsureLoggedIn(), api.CreateService)
				serviceRoutes.PUT("", middleware.EnsureLoggedIn(), api.UpdateService)
			}
			...
			{
				runbookRoutes.GET("", middleware.EnsureLoggedIn(), api.GetRunbooks)
				runbookRoutes.DELETE("", middleware.EnsureLoggedIn(), api.DeleteRunbooks)
				runbookRoutes.POST("", middleware.EnsureLoggedIn(), api.CreateRunbook)
				runbookRoutes.PUT("", middleware.EnsureLoggedIn(), api.UpdateRunbook)
			}
			...
			alertRoutes := v1Routes.Group("/alert")
			{
				...
				alertRoutes.PUT(":id/kernel", middleware.EnsureLoggedIn(), api.StartAlert)
			...
			historyRoutes := v1Routes.Group("/history")
			{
				...
				historyRoutes.GET("", middleware.EnsureLoggedIn(), api.GetHistories)
			}
			releaseRoutes := v1Routes.Group("/release")
			{
				...
				releaseRoutes.PUT("", middleware.EnsureLoggedIn(), api.CreateRelease)
				// releaseRoutes.PUT("", middleware.EnsureLoggedIn(), api.UpdateRelease)
				releaseRoutes.PUT("/:release_id", middleware.EnsureLoggedIn(), api.PromoteRelease)
			}
		}
	}
	...
	uiRoutes := router.Group("/ui", middleware.CORSMiddleware())
	{
		...
		uiRoutes.GET("/releases", middleware.EnsureLoggedIn(), page.ReleasesPageEndpoint)
		// uiRoutes.GET("/releases/:team/:project", middleware.EnsureLoggedIn(), page.ReleasesTeamProjectPageEndpoint)
		uiRoutes.GET("/releases/:release_id", middleware.EnsureLoggedIn(), page.ReleasePageEndpoint)
		uiRoutes.GET("/releases/:release_id/:run_id", middleware.EnsureLoggedIn(), page.RunPageEndpoint)
		uiRoutes.GET("/releases/:release_id/:run_id/:service", middleware.EnsureLoggedIn(), page.WorkflowPageEndpoint)
	}

	htmxRoutes := router.Group("/htmx", middleware.CORSMiddleware(), middleware.EnsureLoggedInAPI())
	{
		projectsRoutes := htmxRoutes.Group("/projects")
		{
			projectsRoutes.GET("/table", page.ProjectsTableEndpoint)
			projectsRoutes.GET("/search", page.ProjectsSearchEndpoint)
		}
		releasesRoutes := htmxRoutes.Group("/releases")
		{
			releasesRoutes.GET("/table", page.ReleasesTableEndpoint)
			releasesRoutes.GET("/search", page.ReleasesSearchEndpoint)
		}
		releasesTeamProjectRoutes := htmxRoutes.Group("/releases/:team/:project")
		{
			releasesTeamProjectRoutes.GET("/table", page.ReleasesTeamProjectTableEndpoint)
			releasesTeamProjectRoutes.GET("/search", page.ReleasesTeamProjectSearchEndpoint)
		}
		releaseRoutes := htmxRoutes.Group("/release/")
		{
			releaseRoutes.GET("/table/:release_id", page.ReleaseTableEndpoint)
			releaseRoutes.GET("/search/:release_id", page.ReleaseSearchEndpoint)
		}
		runRoutes := htmxRoutes.Group("/run/")
		{
			runRoutes.GET("/table/:release_id/:run_id", page.RunTableEndpoint)
			runRoutes.GET("/search/:release_id/:run_id", page.RunSearchEndpoint)
		}
