...
func RunbookKernelSetup(ctx *gin.Context) {
	...
	rs, err := runbook.GetRunbooks(bson.M{"id": runbookID})
	if err != nil {
		logger.Errorf("", "Could not get runbook with ID %s: %s", runbookID, err)
		ctx.Error(err)
		return
	}
	if len(rs) == 0 {
		logger.Errorf("", "No runbook found with ID %s", runbookID)
		ctx.Error(fmt.Errorf("no runbook found with ID %s", runbookID))
		return
	}
	r := rs[0]
	k := &kernel.Kernel{
	...
	for !k.Ready {
		...
		if iteration == max_iterations {
			ctx.Error(fmt.Errorf("kernel health check timed out with ID %s", k.ID))
			return
		}
		iteration += 1
	}
...
func DeleteRunbooks(ctx *gin.Context) {
	filter := buildQuery(ctx)

	if err := runbook.DeleteRunbooks(filter); err != nil {
		utils.Error(err, ctx, http.StatusInternalServerError)
		return
	}
...
func GetRunbooks(ctx *gin.Context) {
	filter := buildQuery(ctx)

	runbooks, err := runbook.GetRunbooks(filter)
...
func UpdateRunbook(ctx *gin.Context) {
	var r runbook.Runbook
	if err := ctx.ShouldBindJSON(&r); err != nil {
		utils.Error(err, ctx, http.StatusInternalServerError)
		return
